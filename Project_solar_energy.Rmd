...
```{r}
sum(capacity_potential_lmi[,4:15]<0)
```
...
#counties with from more than >= 50% value of zero values 
```{r}
capacity_potential_lmi[capacity_potential_lmi$COUNT>5,]
```

#remove counties with from more than >= 50% value of zero values 
```{r}
...
```{r}
capacity_potential_lmi_clean[capacity_potential_lmi_clean$COUNT>5,]
```
```{r}
dim(capacity_potential_lmi_clean) #13 rows removed
```
...
Vectors of names of states and counties
```{r}
...
```
*Take note*: potential outliers: California_LosAngeles 
...
```{r}
#View(num_vabs_cp_lmi.s)
dim(num_vabs_cp_lmi.s)
```
...
There is a contrast in solar rooftop potential capacity in 2 groups: multifamily-own and single-family-rent for LMI households. *need to understand more details what is multifamily/single family and rent/own*
For Texas and California, there is a higher focus of solar rooftop capacity potential in single-family-rent. *WHY?*
...
Extract numerical parts - *states only*
# Average solar rooftop energy capacity per county for each state
...
##Community sharing and Personal use 
